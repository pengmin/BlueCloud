...
{
	class MiPIOutRepository : ICertificateOutRepository
	{
		private AdoProxy _ado = null;

		public MiPIOutRepository(AdoProxy ado)
		{
			this._ado = ado;
		}
		public Certificate GetCertificate(string id)
		{
			...
			var sql = @"select DBill,DChk,StockId,StockName,SupID,AllName,MainAmtTPur from MiPIMain where BillNo='{0}'";
			try
			{
				this._ado.Open();
